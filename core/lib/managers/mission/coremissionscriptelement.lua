...
Hooks:PreHook(MissionScriptElement, "_calc_base_delay", "mpga_calc_base_delay", function(self)
  ...
  if not MPGA or not MPGA.settings then
    ...
    if (Global.game_settings.level_id == "rat" or
        Global.game_settings.level_id == "alex_1" or
...
Hooks:PreHook(MissionScriptElement, "_calc_element_delay", "mpga_calc_element_delay", function(self, params)
  ...
  if not MPGA or not MPGA.settings then
    ...
    if ((Global.game_settings.level_id == "rat" or
         Global.game_settings.level_id == "alex_1" or
         Global.game_settings.level_id == "ratdaylight") and
        self._id == 100724) or
       (Global.game_settings.level_id == "mex_cooking" and
        self._editor_name == "counter_below3") or
       ((Global.game_settings.level_id == "mia_1" or
         Global.game_settings.level_id == "crojob2") and
        self._editor_name == "timer_to_next") then
...
if (not Global.game_settings.level_id == "rat" or
    not Global.game_settings.level_id == "alex_1" or
    not Global.game_settings.level_id == "ratdaylight") then
  return
end

local MPGA = _G.MPGA or {}

if not MPGA or not MPGA.settings or not MPGA.settings.twitch then
  return
end
