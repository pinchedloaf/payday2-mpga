...
function PlayerManager:refresh_stack_counter()
	managers.hud:remove_special_equipment("BagStack")
	if #stack_table > 0 then
		managers.hud:add_special_equipment({id = "BagStack", icon = "pd2_loot", amount = #stack_table})
	end
end

function PlayerManager:can_carry(carry_id)
	if not MPGA:StackEnabled() then
		return master_PlayerManager_can_carry(self, carry_id)
	end

  local check_weight = PlayerManager:MPGAGetCurrentWeight() * MPGA:GetWeightForType(carry_id)
...
function drop_and_set_carry(self, ...)
	...
	if #stack_table > 0 then
      ...
      weight = weight / MPGA:GetWeightForType(cdata.carry_id)
...
function PlayerManager:MPGAGetCurrentWeight()
  return weight
end
